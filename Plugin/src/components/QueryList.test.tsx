...
describe('QueryList', () => {
  ...
  it('renders a list of queries', () => {
    render(<QueryList queries={sampleQueries} onRenameQuery={() => {}} onDeleteQuery={() => {}} />);
  ...
  it('calls onRenameQuery when an edit action is performed on a query', () => {
    ...
    render(<QueryList queries={sampleQueries} onRenameQuery={handleRenameQuery} onDeleteQuery={() => {}} />);
    ...
    expect(handleRenameQuery).toHaveBeenCalledWith(sampleQueries[0].id, 'Updated first query');
  });

  it('calls onDeleteQuery when a delete action is performed on a query', () => {
    const handleDeleteQuery = jest.fn();
    render(<QueryList queries={sampleQueries} onRenameQuery={() => {}} onDeleteQuery={handleDeleteQuery} />);
